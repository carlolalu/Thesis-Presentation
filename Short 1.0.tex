...
My aim: study some of the groups constructed through a special class of them, the invertible finite deterministic Mealy automata, here called simply automata.
\begin{figure}[h]
\centering
\begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
...
\begin{itemize}
	...
	\item $\QQ=\{q_1,\:\ldots\:,q_n\}$ is a finite set called the \alert{set of internal states of the  automaton},
...
\begin{frame}
\frametitle{4.a Actions as tree-automorphisms}
\begin{block}{Definition}
...
\begin{frame}
\frametitle{4.b Actions as tree-automorphisms}
\begin{block}{Proposition}
...
%\end{wrapfigure}
\begin{block}{Proposition}
If $f,g$ are tree-automorphism on $\fslovar$, then $g\circ f$ and $f^{-1}$ are tree-automorphisms on $\fslovar$.
\end{block}
\end{frame}
...
\begin{frame}
\frametitle{5. Groups defined by automata}
\begin{block}{Proposition}
The functions $f:\fslovar\longrightarrow \fslovar$ defined by initial automata (i.e. tree-automorphisms), called \alert{synchronous automatic transformations}, form a group denoted by $\aut_{tree}(\fslovar)$.
\end{block}%\pause
\begin{Definition}
...
%\begin{block}{Definition}
%Let $(B,*_B),(N,*_N)$ be groups and $\varphi:B\longrightarrow \aut(N)$ an homomorphism, where $\aut(N)$ denotes the group of automorphisms on $N$. Then the semidirect product $B\ltimes_\varphi N$ is the group $(B\times N,*_\varphi)$, with composition rule:
%$$\big(b_2,n_2\big)*_\varphi\big(b_1,n_1\big)=\big(\; b_2*_B b_1\;,\; \varphi(b_1)(n_2)*_N n_1\big)$$
%\end{block}
...
\begin{frame}
\frametitle{6. Wreath product}
\begin{block}{Definition}
Let $\symm(\abece)$ be the simmetric group on $\abece=\{x_1,\ldots,x_k\}$. Then the wreath product $\symm(\abece)\wr\aut_{tree}(\fslovar)$ is the group $\big(\symm(\abece)\times\aut_{tree}(\fslovar)^{\abece},\:*\big)$ where the multiplication rule is:
\begin{align*}
&\gamma(c_{x_1},\:\ldots\:,c_{x_k})\:*\:\alpha(a_{x_1},\:\ldots\:,a_{x_k}):=\\
&\gamma\circ\alpha\;(c_{\textcolor{red}{\alpha}(x_1)}\circ a_{x_1},\:\ldots\:,c_{\textcolor{red}{\alpha}(x_k)}\circ a_{x_k})
\end{align*}
\end{block}
\begin{block}{Proposition}
The group of the synchronous automatic transformations $\aut_{tree}(\fslovar)$ is isomorphic to $\symm(\abece)\wr\aut_{tree}(\fslovar)$.
\end{block}
\end{frame}


\begin{frame}
\frametitle{7. Recursive formulas}
\begin{block}{Proposition}
Let $\auto{A}$ be an automaton such that $\QQ=\{q_1,\ldots,q_n\}$ and $\abece=\{x_1,\ldots,x_k\}$. Then $\auto{A}$ is described by $n$ recurrent formulas
\begin{equation*}
\begin{split}
...
Conversely, each such set of $n$ recursive formulas defines an automaton $\auto{A}$ such that $\LAMBDA_{q_l}=f_{q_l}$ for every $q_l\in\QQ$.
\end{block}
\begin{figure}[h]
\begin{tikzpicture}[scale=0.7, every node/.style={transform shape}]
\node [align=center] (aut) at (0,-5 cm) {$\auto{A}$};
\node  [align=center,xshift=7cm] (actions) at (aut) {$\LAMBDA_{q_1}=\lambda_{q_1}(\LAMBDA_{\pi(x_1,q_1)},\ldots,\LAMBDA_{\pi(x_n,q_1)})$\\ $\LAMBDA_{q_2}=\lambda_{q_2}(\LAMBDA_{\pi(x_1,q_2)},\ldots,\LAMBDA_{\pi(x_n,q_2)})$\\ $\vdots$\\ $\LAMBDA_{q_n}=\lambda_{q_n}(\LAMBDA_{\pi(x_1,q_n)},\ldots,\LAMBDA_{\pi(x_n,q_n)})$};
...
\begin{frame}
\frametitle{8. The classification theorem}
\begin{block}{Theorem}
