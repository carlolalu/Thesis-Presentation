...
\end{itemize}
\pause
My aim: study some of the groups constructed through a special class of them, the invertible deterministic Mealy automata, here called automata.
\begin{figure}[h]
\centering
\begin{tikzpicture}[scale=0.8, every node/.style={transform shape}]
...
\begin{itemize}
	...
	\item $\pi:\textcolor{blue}{\abece}\times\QQ \longrightarrow \QQ $ is called the \alert{transition function},
	\item $\lambda:\textcolor{blue}{\abece}\times\QQ \longrightarrow \textcolor{violet}{\abece}$ is such that $\lambda_q=\lambda(\cdot,q):\abece\longrightarrow\abece$ is bijective, and is called the \alert{output function}.
\end{itemize}
\end{block}\pause
\begin{figure}[h!]
\begin{tikzpicture}
...
\begin{block}{Definition}
\centering
$\underline{w}=w_1\ldots w_n$ is a child of $v_1\ldots v_m=\underline{v}$ 

$\Longleftrightarrow$ 

$w_1\ldots w_n=v_1\ldots v_m \textcolor{blue}x$ for some letter $\textcolor{blue}x\in\abece$.
\end{block}
\begin{figure}
\centering
\begin{tikzpicture}[level distance=1cm,
...
\item if $\underline{w}\in\fslovar$ is a child of $\underline{v}$ then $f(\underline{w})$ is a child of $f(\underline{v})$.
\item f is bijective.
\end{itemize}
\end{block}
\begin{figure}
...
%%\begin{block}{Proposition}
%$f,g$ tree-automorphisms on $\fslovar$ $\Rightarrow$ $g\circ f$ and $f^{-1}$ tree-automorphisms on $\fslovar$
%%\end{block}
...
%, called \alert{synchronous automatic permutations},

\begin{frame}
\frametitle{6. Groups defined by automata}
\begin{block}{Proposition}
The functions $f:\fslovar\longrightarrow \fslovar$ defined by initial automata (i.e. tree-automorphisms) form a group denoted by $\aut_{tree}(\fslovar)$.
\end{block}\pause
\begin{Definition}
...
%Let $A$ be a group and $Y$ a set. The \alert{direct sum of $A$ on $Y$} is the group
...
\begin{frame}
\frametitle{7. Wreath product and automata}
\begin{block}{Definition}
Let $\symm(\abece)$ be the simmetric group on $\abece=\{x_1,\ldots,x_k\}$. Then the wreath product $\textcolor{red}{\symm(\abece)}\wr\textcolor{blue}{\aut_{tree}(\fslovar)}$ is the group $\big(\textcolor{red}{\symm(\abece)}\times\textcolor{blue}{\aut_{tree}(\fslovar)^{\abece}},\:*\big)$ where the multiplication rule is:
\begin{align*}
&\textcolor{red}{\gamma}(\textcolor{blue}{c_{x_1}},\:\ldots\:,\textcolor{blue}{c_{x_k}})\:*\:\textcolor{red}{\alpha}(\textcolor{blue}{a_{x_1}},\:\ldots\:,\textcolor{blue}{a_{x_k}}):=\\
&\textcolor{red}{\gamma\circ\alpha}\;(\textcolor{blue}{c_{\textcolor{red}{\alpha}(x_1)}\circ a_{x_1}},\:\ldots\:,\textcolor{blue}{c_{\textcolor{red}{\alpha}(x_k)}\circ a_{x_k}})
\end{align*}
\end{block}
\pause
\begin{block}{Proposition}
...
\begin{frame}
\frametitle{8. System of formulas}
\begin{block}{Proposition}
...
\begin{frame}
\frametitle{9. The classification theorem}
\begin{block}{Theorem}
...
\pause
\item $\Z$,
\item the infinite dihedral group $\mathcal{D}_\infty=(\Z_2\times\Z,*)$, where:
$$(\textcolor{red}h,z_1)*(\textcolor{red}k,z_2)=(\textcolor{red}{h+_{\Z_2}k},(-1)^{\textcolor{red}k}z_1+z_2),$$
\pause
\item the lamplighter group $\LL=(\Z\times\Z_{2}^{(\Z)},*)$, where:
$$(\textcolor{red}{z_2},(h_i)_{i\in\Z})*(\textcolor{red}{z_1},(k_i)_{i\in\Z})=(\textcolor{red}{z_2+z_1},(h_{i+\textcolor{red}{z_1}}+_{\Z_2}k_{i})_{i\in\Z}).$$
\end{itemize}
\end{block}
\end{frame}
...
\end{tikzpicture}
\end{figure}
where $x_{ij}\in\{a,b\}$ and $\tau^{i_1},\tau^{i_2}\in\symm(\abece)=\symm(\{0,1\})$. There are 64 possibilities. We proceed by analysing part of them.
\end{block}
%\pause
